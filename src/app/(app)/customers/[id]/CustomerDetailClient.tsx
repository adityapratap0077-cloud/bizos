...
export default function CustomerDetailClient({
  ...
  const taskColumns: Column<Task>[] = [
    { header: 'Title', render: (t) => <span className="font-medium text-gray-900">{t.title}</span> },
  ...
  const bookingColumns: Column<Booking>[] = [
    { header: 'Service', render: (b) => <span className="font-medium text-gray-900">{b.service}</span> },
    ...
    },
    {
      header: 'Price',
      render: (b) =>
        b.price != null ? formatMoney(b.price, businessCurrency) : <span className="text-gray-400">—</span>,
    },
    {
      header: 'Status',
  ...
  const invoiceColumns: Column<Invoice>[] = [
    { header: 'Number', render: (inv) => <span className="font-medium text-gray-900">{inv.invoice_number}</span> },
    ...
    <div>
      ...
      <Card className="mb-6">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-gray-400">Customer since</p>
      ...
      {/* Related lead */}
      <Card className="mb-6">
        <h2 className="mb-3 text-base font-semibold text-gray-900">Related lead</h2>
        {lead ? (
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
            <span className="font-medium text-gray-900">{lead.name}</span>
            <Badge color={LEAD_BADGE_COLORS[lead.status]}>{lead.status}</Badge>
            {lead.service_interested && <span className="text-gray-600">{lead.service_interested}</span>}
      ...
      {/* Tasks */}
      <Card className="mb-6">
        <h2 className="mb-3 text-base font-semibold text-gray-900">Tasks</h2>
        {tasks.length === 0 ? (
          <p className="text-sm text-gray-400">No tasks for this customer yet.</p>
        ) : (
      ...
      {/* Bookings */}
      <Card className="mb-6">
        <h2 className="mb-3 text-base font-semibold text-gray-900">Bookings</h2>
        {bookings.length === 0 ? (
          <p className="text-sm text-gray-400">No bookings for this customer yet.</p>
        ) : (
      ...
      {/* Invoices */}
      <Card className="mb-6">
        <h2 className="mb-3 text-base font-semibold text-gray-900">Invoices</h2>
        {invoices.length === 0 ? (
      ...
      {/* Activity */}
      <Card>
        <h2 className="mb-3 text-base font-semibold text-gray-900">Activity history</h2>
        {activity.length === 0 ? (
          <p className="text-sm text-gray-400">No recorded activity for this customer yet.</p>
        ) : (
          <ul className="divide-y divide-gray-100">
            {activity.map((a) => (
              <li key={a.id} className="flex items-start justify-between gap-4 py-2.5">
                <p className="text-sm text-gray-700">{a.action}</p>
                <span className="shrink-0 text-xs text-gray-400" title={formatDate(a.created_at)}>
